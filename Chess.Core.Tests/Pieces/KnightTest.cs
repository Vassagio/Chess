...
namespace Chess.Core.Tests.Pieces {
    public class KnightTest {        
        [Fact]
        public void Creates_A_New_Knight() {
            var knight = new Knight(new List<IMovement>());

            Assert.IsAssignableFrom<Piece>(knight);
        }

        [Fact]
        public void Returns_Possible_Moves_When_Knight_One_Movement() {
            ...
            var knight = new Knight(movements);

            var moves = knight.GetMovesFrom(startingLocation);
        ...
        [Fact]
        public void Returns_Possible_Moves_When_Knight_Multiple_Movements() {
